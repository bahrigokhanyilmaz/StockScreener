...
// Today's date in US Pacific time (America/Los_Angeles), as YYYY-MM-DD.
// The backend stores first_tracked as a Pacific calendar date, so we compare
// against "today in PT" — not the browser's local/UTC day — or the Days count
// drifts by one around midnight.
...
function getDaysTracked(firstTracked: string | null | undefined): string {
  ...
  // Both dates are Pacific calendar dates (YYYY-MM-DD). Diff by whole days using
